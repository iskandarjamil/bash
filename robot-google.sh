...
for i in ${!sites[@]}; do
  ...
  curl --compressed -H "Range: bytes=0-524288" -H "Cache-Control: no-cache" -H "Connection: close" -A "Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)" ${sites[i]}
  echo ""
done
